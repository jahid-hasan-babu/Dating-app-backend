    ...
    ws.on('message', async (data: string) => {
      try {
        ...
        switch (parsedData.type) {
          ...
          case 'offer': {
            const { offer, senderId, receiverId } = parsedData;

            // Send the offer to the receiver (other user)
            wss.clients.forEach((client: ExtendedWebSocket) => {
              if (
                client.userId === receiverId &&
                client.readyState === WebSocket.OPEN
          ...
          case 'answer': {
            const { answer, senderId, receiverId } = parsedData;

            // Send the answer to the sender (to complete the connection)
            wss.clients.forEach((client: ExtendedWebSocket) => {
              if (
          ...
          case 'iceCandidate': {
            const { candidate, senderId, receiverId } = parsedData;
